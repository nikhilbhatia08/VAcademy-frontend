...
import { GET_COURSES_OF_A_USER } from '../components/utils';
    ...
    const [courses, setCourses] = useState([]);
      const handleFetch = async() => {
            ...
            const data = {
                ...
                name: JSON.parse(userId).name,
            }
            console.log(data);
            try{
                const res = await axios.get(GET_COURSES_OF_A_USER + '/' + JSON.parse(userId).id);
                ...
                courses?.map((course, index) => {
                    return(
                        <View className="mb-3" key={index}>
                            <CourseCard imgUrl={course.course_image} id={course.course_id} key={index} Title={course.course_name} price={course.price}/>
                        </View>
                    )
                })
            }
